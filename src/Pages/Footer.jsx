...
import GitHuub from "../Images/gitHub (2).png";
import "./Footer.css";
import { useMediaQuery } from "react-responsive";

const Footer = () => {
  const isMobile = useMediaQuery({ query: "(max-width: 768px)" });
  const gitLink = "https://github.com/juttjoshwa";
  const NewTab = async (l) => {
    window.open(l);
  };

  const [boxes, setBoxes] = useState(
    Array.from({ length: 375 }, (_, index) => index + 1)
  );
  ...
  return (
    <div className="GitContainer">
      <h1 className="GitTitle">Want to see all Projects!</h1>
      {isMobile && isMobile ? (
        <div className="gitHub-Img-container-for-mobile">
          <img
            onClick={() => NewTab(gitLink)}
            src={GitHuub}
            alt="Git"
            className="GitHub-Img-Mobile"
          />
        </div>
      ) : (
          ...
          <div className="boxContainer">
            {boxes.map((index) => (
              <div
                ...
                className={`box ${activeBoxes.includes(index) ? "active" : ""}`}
              ></div>
            ))}
          </div>
